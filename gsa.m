...
GSAp := function(poly,galois_group,galois_data,lattice:search_bound:=10^5,skip:=[])
	...
	ClassifyNode := procedure(node,~realized,~unknown,~rat_pts,~proved_finite,~proved_infinite,~parametrized,~finite_meets)
		h := node; H := Group(h);
		...
		if h in skip then
			...
			if depth ne 0 then
				...
				if is_below_finite_node or is_below_finite_meet then
					...
					ClassifyNode(h,~realized,~unknown,~rat_pts,~proved_finite,~proved_infinite,~parametrized,~finite_meets);
				end if;
			end if;
		end if;
		Remove(~queue,1);
